...
# run app
java -jar target/todd-test-three-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t parsontodd/todd-test-three:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/todd-test-three -n dev

# get external endpoint
kubectl get svc todd-test-three-svc -n dev
```
...
```bash
kubectl port-forward svc/todd-test-three-svc 8080:80 -n dev
...
kubectl rollout restart deploy/todd-test-three -n dev
kubectl rollout status deploy/todd-test-three -n dev
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/toddtestthree/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/toddtestthree/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
